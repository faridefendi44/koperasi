                    ...
                    <tbody>
                        @foreach ($members as $item)
                            <tr class="bg-[#D9D9D9] border-b    ">
                                ...
                                <td class="px-6 py-4">
                                    {{ Str::words($item->alamat, 5, '...') }}
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex space-x-3 ">
                                        @if ($item->status == 'proses')
                                            <div class="">
                                                <button id="reject" data-modal-target="popup-reject-{{ $item->id }}"
                                                    data-modal-toggle="popup-reject-{{ $item->id }}"
                                                    class="px-3 py-2 bg-[#BF3131] text-white rounded-md">Ditolak</button>
                                            </div>
                                            <div class="">
                                                <button id="approve" data-modal-target="popup-approve-{{ $item->id }}"
                                                    data-modal-toggle="popup-approve-{{ $item->id }}"
                            ...
                            <div id="popup-approve-{{ $item->id }}" tabindex="-1" aria-labelledby="modal-title"
                                ...
                                <div class="relative p-4 w-full max-w-md max-h-full">
                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                        <button type="button"
                                            ...
                                            data-modal-hide="popup-approve-{{ $item->id }}">
                                        ...
                                        <div class="p-4 md:p-5 text-center">
                                            ...
                                            <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Anda
                                                ...
                                                anggota ini?</h3>
                                            <form action="{{ route('member.approve', $item->id) }}" method="POST">
                                                @csrf
                                                @method('PUT')
                                                ...
                                                <button data-modal-hide="popup-approve-{{ $item->id }}"
                                                    type="button"
                            ...
                            <div id="popup-reject-{{ $item->id }}" tabindex="-1" aria-labelledby="modal-title"
                                ...
                                <div class="relative p-4 w-full max-w-md max-h-full">
                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                        <button type="button"
                                            class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                            data-modal-hide="popup-reject-{{ $item->id }}">
                                            <svg class="w-5 h-5 closeModal " aria-hidden="true"
                                                xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                                <path stroke="currentColor" stroke-linecap="round"
                                                    stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <div class="p-4 md:p-5 text-center">
                                            ...
                                            <form action="{{ route('member.reject', $item->id) }}" method="POST">
                                                @csrf
                                                @method('PUT')
                                                <button data-modal-hide="popup-modal" type="submit"
                                                ...
                                                <button data-modal-hide="popup-reject-{{ $item->id }}" type="button"
                                                    class="closeModal py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
                                                    Batal</button>
                                            </form>
                                        </div>
