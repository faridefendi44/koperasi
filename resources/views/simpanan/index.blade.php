                        ...
                        {{-- @foreach ($simpanans->unique('id_anggota') as $item)
                            @php
                                ...
                                $simpananPokok = $simpanans->where('id_anggota', $item->id_anggota)->first()
                                    ->simpanan_pokok;
                                $totalSimpanan = $item->anggota->simpanan + $totalSimpananWajib + $simpananPokok;
                            @endphp
                            <tr class="bg-[#D9D9D9] border-b">
                                <th scope="row"
                                    class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <!-- Add any unique identifier if needed -->
                                </th>
                                <td class="px-6 py-4">
                                    {{ $item->id_simpanan }}
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    {{ 'Rp ' . number_format($item->anggota->simpanan, 0, ',', '.') }}
                                </td>
                                <td class="px-6 py-4">
...
@else
    @foreach ($simpanans->unique('id_anggota') as $item)
        @php
            ...
            if (!$anggota) {
                continue; // Skip this iteration if member is null
            }
            $totalSimpananWajib = $simpanans
                ->where('id_anggota', $item->id_anggota)
                ->sum('simpanan_wajib');
